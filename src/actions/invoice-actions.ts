...
// ============================================================
// 見積書作成（オプション自由選択版・契約詳細から）
// ============================================================

export interface EstimateWithOptionsInput {
  contract_id: string
  customer_id: string
  plan_id: string
  option_ids: string[]
  custom_options: { name: string; monthly_fee: number }[]
  include_initial_fees: boolean
  initial_fees: { label: string; amount: number; quantity: number }[]
  notes: string
}

export async function createEstimateWithOptions(
  input: EstimateWithOptionsInput
): Promise<ActionResult<{ id: string }>> {
  const supabase = await createClient()
  ...
  revalidatePath('/admin/invoices')
  revalidatePath(`/admin/contracts/${input.contract_id}`)
  return { success: true, data: { id: invoice.id } }
}
